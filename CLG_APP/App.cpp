...
void ITEM::convert()
{
    start.x = 10 * round(item.start.x / 10);
    start.y = 10 * round(item.start.y / 10);
    end.x = 10 * round(item.end.x / 10);
    end.y = 10 * round(item.end.y / 10);
}
int ITEM::area()
{
...
void App()
{
    ...
    ImGui::Text("Kursor  %f     \t \t  %f", io.MousePos.x - ve2[0], io.MousePos.y - ve2[1]);
    //
